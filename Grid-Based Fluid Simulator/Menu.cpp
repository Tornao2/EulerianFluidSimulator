...
void Menu::drawMenu() {
    for (int i = 0; i < 2; i++)
        for (int j = 0; j < 4; j++) {
            DrawRectangle(x + (j * width / 4) + (width / 16), y + i * 80 + height / 20, width / 8, width / 8, BLACK);
            DrawRectangle(x + (j * width / 4) + (width / 12), y + i * 80 + height / 20 + width / 48, width / 12, width / 12, colorArray[i * 2 + j + 1]);
            DrawText(TextFormat("%i", i * 2 + j + 1), x + (j * width / 4) + (width / 8) - MeasureText(TextFormat("%i", i * 2 + j + 1), FONTSIZE) / 2, y + i * 80 + height / 20 + width / 8, FONTSIZE, selectedColor == i * 2 + j + 1 ? DARKGREEN : BLACK);
        }   
    ...
    DrawRectangle(x + width * 6 / 8, height * 13 / 32, width / 8, width / 8, textField == 4 ? SELECTCOLOR : WHITE);
    DrawText(TextFormat("%i", brushSize), x + width * 6 / 8 + MeasureText(TextFormat("%i", brushSize), FONTSIZE) / 2, height * 13 / 32, FONTSIZE, BLACK);
